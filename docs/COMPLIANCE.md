...
The compliance system has three layers:
...
3. **Group Policy ACL Audit** — Reviews Meraki network group policies for missing custom ACLs and overly-broad `allow any/any` L3 rules.
...
The compliance playbook now also audits network group policies:
...
```bash
ansible-playbook playbooks/compliance_check.yml -i inventory/production.yml --tags group-policy-acl
```
...
| `group_policy_acl_audit_enabled` | `true` | Enable/disable group policy ACL audit checks |
