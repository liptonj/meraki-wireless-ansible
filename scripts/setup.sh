...
# One-command setup: creates venv, installs dependencies, installs collections
#
# Usage: ./scripts/setup.sh
# Or: make setup
#
# This script is beginner-friendly and includes helpful output messages
# Perfect for YouTube tutorial viewers following along
...
# Check Python version (need 3.12+ for ansible-core 2.20.2)
...
if [ ! -f ".env" ]; then
    if [ -f ".env.example" ]; then
        ...
        cp .env.example .env
        warning "⚠️  IMPORTANT: Please edit .env and add your Meraki API credentials!"
